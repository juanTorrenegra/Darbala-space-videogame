...
class _DebugMenuState extends State<DebugMenu> {
  ...
  /// Global scale for the whole debug menu (1.5 = 50% bigger).
  static const double _menuScale = 1.5;
  ...
  Widget _buildDebugDrawer() {
    return Container(
      // Size in the game's fixed design space (1560×720), NOT MediaQuery:
      // the whole game sits inside a FittedBox, so MediaQuery returns the
      // real screen size and double-scales the menu (tiny on the phone,
      // huge on web). Using logical size keeps it 80% of the game height
      // on every platform.
      width: MyGame.logicalWidth * 0.224, // 20% narrower than before
      // Divide by the menu scale so the scaled drawer lands exactly at
      // 0.8 of the screen height instead of overflowing.
      height: MyGame.logicalHeight * 0.8 / _menuScale,
